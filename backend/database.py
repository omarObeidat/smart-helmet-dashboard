...
database.py
------------
SQLite database layer: table definitions, worker CRUD, and sensor
...
def get_connection():
    """New DB connection with foreign keys enabled.

    WAL mode allows concurrent reads/writes - needed because the ESP32
    POSTs every ~2s while the dashboard runs several GETs every 2s;
    without WAL this can throw random "database is locked" errors.
    busy_timeout gives a brief lock up to 3s to clear before failing.
    """
...
def init_db():
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS workers (
            ...
            -- Medical info: display-only during emergencies, not used by the AI
            blood_type TEXT,
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS sensor_readings (
            ...
            gas_ppm REAL,            -- raw MQ135 ADC reading (0-4095); name kept for compatibility
            temperature REAL,
            humidity REAL,
            accel_x REAL,
            accel_y REAL,
            accel_z REAL,
            helmet_worn INTEGER,     -- 1 = worn, 0 = removed
            buzzer_on INTEGER DEFAULT 0,  -- buzzer state at time of reading (syncs dashboard LED)

            air_quality_status TEXT,   -- Safe / Moderate / Dangerous
            fall_status TEXT,          -- idle / motion / step / fall
            base_risk_score REAL,
            final_risk_score REAL,     -- after age/health multiplier
    ...
    # Pending remote commands per helmet; ESP32 checks this on every periodic POST
    cur.execute("""
        CREATE TABLE IF NOT EXISTS device_commands (
            helmet_id TEXT PRIMARY KEY,
            pending_command TEXT DEFAULT 'none',   -- 'none', 'reset', or 'buzz'
            updated_at TEXT DEFAULT (datetime('now'))
        )
    """)

    # Indexes for the dashboard's frequent polling queries, to avoid full table scans
    cur.execute("""
    ...
    # Migration: old DBs used "gas_index" before switching to raw ADC readings
    ...
    if "gas_index" in existing_columns and "gas_ppm" not in existing_columns:
        ...
        conn.commit()
        print("Migration: renamed gas_index to gas_ppm")
...
def add_worker(data: dict):
    # try/finally: INSERT can fail on a duplicate helmet_id. Without closing
    # the connection on failure, a pending transaction holds a write lock
    # and blocks all later writes (including sensor readings).
    conn = get_connection()
    try:
        cur = conn.cursor()
...
def get_latest_reading(helmet_id: str):
    conn = get_connection()
    cur = conn.cursor()
    # Ordered by reading_id, not timestamp: datetime('now') has 1s resolution,
    # so two readings in the same second could otherwise sort unpredictably.
    cur.execute("""
...
def get_readings_history(helmet_id: str, limit: int = 100):
    """Returns the last N readings, oldest first, for charting."""
    conn = get_connection()
    cur = conn.cursor()
    cur.execute("""
        SELECT * FROM sensor_readings
...
def delete_readings(helmet_id: str = None):
    """Clears sensor readings (dashboard "Reset Readings" button).
    Clears one helmet's readings if given, otherwise all readings."""
    conn = get_connection()
    cur = conn.cursor()
    if helmet_id:
...
def get_all_readings(helmet_id: str = None):
    """Returns all readings (no limit), used for Excel export."""
    conn = get_connection()
    cur = conn.cursor()
    if helmet_id:
        cur.execute("""
...
def has_recent_incident(helmet_id: str, incident_type: str, cooldown_seconds: int = 60) -> bool:
    """Checks for a same-type incident within the last N seconds, to avoid
    alert flooding from an ongoing condition (e.g. dangerous air) that would
    otherwise log a new incident with every ~2s reading."""
    conn = get_connection()
    cur = conn.cursor()
    cur.execute("""
...
def get_all_helmet_ids():
    """Returns all known helmet IDs: registered to a worker, or any that
    have already sent readings even if not yet linked to a worker."""
    conn = get_connection()
    cur = conn.cursor()
    cur.execute("""
...
def delete_all_incidents():
    """Clears the incident log (dashboard "Clear Alerts" button)."""
    conn = get_connection()
    cur = conn.cursor()
...
# ---------------------------------------------------------------------------
# Pending device commands (remote reset/buzz)
# ---------------------------------------------------------------------------

def set_pending_command(helmet_id: str, command: str):
    """Stores a pending command for a helmet, executed on its next periodic POST."""
    conn = get_connection()
    cur = conn.cursor()
    cur.execute("""
...
def get_and_clear_pending_command(helmet_id: str) -> str:
    """Returns the pending command for a helmet and resets it to 'none'."""
    conn = get_connection()
    cur = conn.cursor()
